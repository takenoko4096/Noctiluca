...
enum class SelectorType(val aliveOnly: Boolean, vararg defaultArguments: SelectorArgument) {
    ...
    P(true, SelectorArgument.Sort(SelectorSortOrder.ARBITRARY), SelectorArgument.Type(EntityTypes.PLAYER), SelectorArgument.Limit(1)),
    R(true, SelectorArgument.Sort(SelectorSortOrder.RANDOM), SelectorArgument.Type(EntityTypes.PLAYER), SelectorArgument.Limit(1)),
    A(false, SelectorArgument.Sort(SelectorSortOrder.ARBITRARY), SelectorArgument.Type(EntityTypes.PLAYER)),
    ...
    fun getCandidates(stack: NoctilucaCommandSourceStack): List<Entity> {
        val entities = stack.sender.getServer().allLevels.flatMap { it.allEntities }.filter { it.type != EntityTypes.PLAYER }
